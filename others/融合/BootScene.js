...
export default class BootScene extends Phaser.Scene {
  ...
  registerHeroAnims() {
    ...
    // —— 高质量近战连击 attack_pro：侧视，朝右（左方向 flipX 镜像）——
    // 注意：原素材 34.png 是与 idle 相同的"收尾立姿"，但其帧画布尺寸与 idle 表完全不同，
    // 在固定 scale 下会显示明显过大。直接移除最后一帧，动画结束后由 update 切回 hero_knife_idle，
    // 视觉上无缝衔接静息姿态。
    if (this.textures.exists('hero_pro_attack_1')) {
      ...
      // 总时长 ~ 420ms：均分每帧 15ms，命中关键帧（5 / 12 / 19 = 出剑高潮）略加长
      const proFrames = proIndices.map((i) => {
        let duration = 15;
        if (i === 5 || i === 12 || i === 19) duration = 35; // 拔剑/挥剑高潮
        else if (i === 1 || i === 28) duration = 25; // 起势 / 收尾
