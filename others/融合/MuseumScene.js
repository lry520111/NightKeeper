  ...
  update() {
    ...
    if (this._useHeroPlayer) {
      ...
      if (this._useBowHero) {
        ...
        // 4-directional shoot anims 现在 left 方向使用本身朝左的 hero_bow_3_reflect，
        // 因此不再需要 flipX；只有 legacy 单向 hero_bow_shoot 仍需按 attackDir flipX。
      ...
      // 持刀主角攻击优先使用 attack_pro 高质量连击动画（侧视，按朝向 flipX；上下方向沿用当前面向）
      const useProAttack = useDirectional && attacking
        && this.anims.exists('hero_pro_attack');
  ...
  tryPlayerAttack() {
    ...
    if (this._useHeroPlayer) {
      if (this._useKnifeHero) {
        ...
        // 所有方向都使用高质量 attack_pro 多帧连击（侧视，左 flipX；
        // 上/下方向也统一使用侧视动作，按水平分量选面向，避免与原扇形贴图不一致）
    ...
    if (this._useHeroPlayer) {
      if (this._useBowHero) {
        ...
        if (this.anims.exists(dirShootKey)) {
          // 4 向 shoot 帧已包含正确朝向（left 用 hero_bow_3_reflect），无需 flipX
          this.player.setFlipX(false);
